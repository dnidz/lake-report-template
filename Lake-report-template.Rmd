...
if(is.weekly) { 
  lake.plot(data.L1=weekly.data,data.L2=L2.data,wy.st=T,lake=params$lake,year=params$year,y=y)
} else {
  lake.plot(data.L1=weekly.data,data.L2=L2.data,wy.st=F,lake=params$lake,year=params$year,y=y)
}
...
table.trends<-trends.sig %>%
  ...
  kable(col.names=c("Parameter","Change per Decade"),
        align="lr"
  ) %>%
...
`r if(has.alk.trend) paste("The dashed black line shows the statistically discernable trend (p<0.05) in alkalinity.")`
...
`r if(has.UV.trend) paste("The dashed black line shows the statistically discernable trend (p<0.05) in UV absorbance.")`
